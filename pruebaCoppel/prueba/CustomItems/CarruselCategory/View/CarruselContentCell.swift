...
class CarruselContentCell: UICollectionViewCell {
    static let identifier = "CarruselContentCell"
    ...
    private lazy var contentViewContainer: UIView = {
        let view = UIView()
        view.translatesAutoresizingMaskIntoConstraints = false
        [circularBackgroundView, labelTitle].forEach(view.addSubview(_:))

        NSLayoutConstraint.activate([
            circularBackgroundView.heightAnchor.constraint(equalToConstant: 90),
            circularBackgroundView.widthAnchor.constraint(equalToConstant: 90),
            ...
            labelTitle.topAnchor.constraint(equalTo: circularBackgroundView.bottomAnchor, constant: 8),
    ...
    func configure(with carruselModel: CarruselContentCellModel) {
            ...
            let task = URLSession.shared.dataTask(with: url) { data, response, error in
                ...
                DispatchQueue.main.async {
                    if let image = UIImage(data: data) {
                        ...
                        self.labelTitle.text = carruselModel.title
                    }
                }
            }
